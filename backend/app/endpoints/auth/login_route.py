...
from app.models.types.auth_types import (
    LoginForm,
    UserCreate,
    Change_password,
    SetNewPassword,
...
async def login(
    ...
    login_form: LoginForm = LoginForm(username=username, password=password)
    token = authentication.login(data=login_form, session=session)
    if token:
